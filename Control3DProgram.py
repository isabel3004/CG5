...
class GraphicsProgram3D:
    ...
    def __init__(self):
        ...
        particle_effect_01 = ParticleEffect(Point(-5.0, 3.0, -3.0), self.texture_id_particle, 1.0)
        self.fire_01 = Fire(self.sprite_shader, self.shader, particle_effect_01, 1)
        particle_effect_02 = ParticleEffect(Point(-5.0, 3.0, 3.0), self.texture_id_particle, 1.0)
        self.fire_02 = Fire(self.sprite_shader, self.shader, particle_effect_02, 2)
        particle_effect_03 = ParticleEffect(Point(-1.0, 3.0, 0.0), self.texture_id_particle, 1.0)
    ...
    def update(self):
        ...
        if self.fr_sum > 1.0:
            print('framerate =', self.fr_ticker / self.fr_sum)
            self.fr_sum = 0
            self.fr_ticker = 0
        if not self.game:
        ...
        # actual game
            ...
            if (3.6 <= self.view_matrix.eye.x <= 4.2 and -5.5 <= self.view_matrix.eye.z <= -4.5) or (-1.5 <= self.view_matrix.eye.x <= 1.5 and -9.5 <= self.view_matrix.eye.z <= -9):
                self.boost = True
                self.tb = time.time()
            if self.boost and (time.time()-self.tb)<0.8:
                self.view_matrix.slide(0, 0, -3 * delta_time)
            
            # collision and check passing of ports
            def port1check(x, z, nr):
                c=0.15
                if x+0.1+c>=self.view_matrix.eye.x>=x-0.1-c and z-0.1-c <=self.view_matrix.eye.z<=z+1.1+c:
                    if z+0.9-c <= self.view_matrix.eye.z or self.view_matrix.eye.z <= z+0.1+c:
            ...
            def port2check(x, y, nr):
                c = 0.15
                if x+0.1+c>=self.view_matrix.eye.x>=x-1-c and y-0.1-c <=self.view_matrix.eye.z<=y+0.1+c:
                    if x-0.1-c <= self.view_matrix.eye.x or self.view_matrix.eye.x <= x-0.9+c:
                        self.view_matrix.eye.x = past_position.x
                        self.view_matrix.eye.y = past_position.y
                        self.view_matrix.eye.z = past_position.z
